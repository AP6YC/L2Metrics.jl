...
This page serves as the contribution guide for the `L2Metrics.jl` package.
...
The main point of contact is the [GitHub issues](https://github.com/AP6YC/L2Metrics/issues) page for the project.
...
```sh
git clone git@github.com:AP6YC/L2Metrics.jl.git
julia --project=.
```
...
```julia-repl
julia> ]
(L2Metrics) pkg> instantiate
(L2Metrics) pkg> test
```

## GitFlow

The `L2Metrics.jl` package follows the [GitFlow](https://nvie.com/posts/a-successful-git-branching-model/) git working model.
...
3. When you are satisfied with your changes, initiate a [GitHub pull request](https://github.com/AP6YC/L2Metrics.jl/pulls) (PR) to merge the feature branch with `develop`.
...
The `L2Metrics.jl` project has the following file structure:
